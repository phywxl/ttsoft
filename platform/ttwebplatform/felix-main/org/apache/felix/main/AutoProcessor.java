...
public class AutoProcessor
{
    ...
    **/
    ...
    private static void processAutoProperties(Map configMap, BundleContext context)
    {
        ...
        /* phywxl modify 20130703, …Ë÷√∆Ù∂ØÀ≥–Ú. BEGIN. ◊¢“‚£∫◊¢ Õ¡À…œ ˆ¥˙¬Î */
        List<String> cfs = new ArrayList<String>();
        for (Iterator i = configMap.keySet().iterator(); i.hasNext(); ) {
        	String key = ((String) i.next()).toLowerCase();
            if (key.startsWith(AUTO_START_PROP) || key.startsWith(AUTO_INSTALL_PROP)) {
            	cfs.add(key);
            }
        }
        java.util.Collections.sort(cfs, new java.util.Comparator<String>(){
			@Override
        ...
        /* phywxl modify 20130703, …Ë÷√∆Ù∂ØÀ≥–Ú. END */
        
        /* phywxl modify 20130703, …Ë÷√∆Ù∂ØÀ≥–Ú. BEGIN */
        /*List<String> cfs = new ArrayList<String>();
        for (Iterator i = configMap.keySet().iterator(); i.hasNext(); ) {
        	String key = ((String) i.next()).toLowerCase();
        ...
        System.out.println("====AutoProcessor.processAutoProperties(Map,BundleContext) keys=" + cfs);
        for (int i = 0; i < cfs.size(); i++) {
        	String key = cfs.get(i);
        	StringTokenizer st = new StringTokenizer((String) configMap.get(key), "\" ", true);
            for (String location = nextLocation(st); location != null; location = nextLocation(st))
        ...
        for (Iterator i = configMap.keySet().iterator(); i.hasNext(); )
        {
            ...
            if (key.startsWith(AUTO_START_PROP))
            {
                ...
                for (String location = nextLocation(st); location != null; location = nextLocation(st))
                {
                    ...
                    try
                    {
                    	//System.out.println("====AutoProcessor.processAutoProperties(Map,BundleContext) key=" + key + ", location=" + location);
                        Bundle b = context.installBundle(location, null);
                        if (b != null)
                        {
                            b.start();
